import React, { useState, useEffect } from 'react';

const SPLINE_SRC =
  'https://my.spline.design/dunes-r4IlewzoPYCEeTp6o7lQCo2g/';

/**
 * Spline 3D는 그대로 — 색감만 aurora-bg.jpg처럼 밝게:
 * iframe에 filter(brightness/saturate/contrast) + 오로라 블렌드 레이어.
 */
const SplineBackground: React.FC = () => {
  const [isLoaded, setIsLoaded] = useState(false);
  const [mountIframe, setMountIframe] = useState(true);

  useEffect(() => {
    const onVisibilityChange = () => {
      if (document.hidden) {
        setMountIframe(false);
        setIsLoaded(false);
      } else {
        setMountIframe(true);
      }
    };

    document.addEventListener('visibilitychange', onVisibilityChange);
    return () =>
      document.removeEventListener('visibilitychange', onVisibilityChange);
  }, []);

  return (
    <div className="fixed inset-0 w-full h-full z-0 overflow-hidden bg-color-of-soul">
      <div
        className={`absolute inset-0 z-[3] flex items-center justify-center transition-opacity duration-1000 ${
          isLoaded ? 'opacity-0 pointer-events-none' : 'opacity-100'
        }`}
        style={{
          background: 'linear-gradient(to bottom, #fffefb, #faf5ff)',
        }}
      >
        <div className="text-sky-500/80 animate-pulse text-sm tracking-widest uppercase font-light">
          Initializing Environment...
        </div>
      </div>

      {mountIframe && (
        <div className="absolute inset-0 z-0 overflow-hidden">
          <iframe
            src={SPLINE_SRC}
            frameBorder="0"
            className={`absolute left-1/2 top-1/2 max-w-none -translate-x-1/2 -translate-y-1/2 border-0
              h-[112vmax] w-[112vmax] min-h-full min-w-full transition-opacity duration-1000
              brightness-[1.24] saturate-[1.38] contrast-[1.06]
              ${isLoaded ? 'opacity-100' : 'opacity-0'}`}
            onLoad={() => setIsLoaded(true)}
            title="3D Dunes Background"
          />
        </div>
      )}

      <div className="aurora-over-spline" aria-hidden />
      <div className="aurora-screen-glow" aria-hidden />

      <div className="pointer-events-none absolute inset-0 z-[1] bg-gradient-to-t from-cyan-50/55 via-white/15 to-rose-50/50" />

      <div
        className="pointer-events-none absolute bottom-0 right-0 z-[2] h-[min(140px,22vh)] w-[min(320px,45vw)] bg-gradient-to-tl from-sky-200/35 via-fuchsia-100/25 to-transparent"
        aria-hidden
      />

      <div
        className="pointer-events-none absolute inset-0 z-[2] bg-color-of-soul-grain"
        aria-hidden
      />
    </div>
  );
};

export default SplineBackground;
